 ...
 */
class Encoder {
    constructor(encoders, defaultEncoder) {
     ...
     */
    encodeMessage(name, value, strict = false) {
        var _a;
        const encoder = ((_a = this.encoders) === null || _a === void 0 ? void 0 : _a.get(name)) || this.defaultEncoder;
        if (encoder !== undefined) {
            ...
            if (!strict && value instanceof Uint8Array) {
                return signature_1.SignatureG1.encodeMessageForSigning(value);
            }
            else {
                throw new Error(`Cannot encode message with name ${name} and value ${value} as neither was any encoder provided nor it was an Uint8Array. Its type was ${typeof value}`);
            }
        }
    }
    /**
     ...
     * @param strict - If set to false and no appropriate encoder is found but the value is a bytearray, it will encode it using the built-in mechanism
     */
    encodeMessageObject(messages, strict = false) {
        const [names, values] = (0, util_1.flattenObjectToKeyValuesList)(messages);
        const encoded = [];
    ...
    encodeDefault(value, strict = false) {
        if (this.defaultEncoder !== undefined) {
            ...
            if (!strict && value instanceof Uint8Array) {
                ...
                throw new Error(`Cannot encode value ${value} as neither was default encoder present nor it was an Uint8Array. Its type was ${typeof value}`);
            }
        }
    }
    /**
     ...
     */
    static positiveIntegerEncoder() {
        return (v) => {
            if (!(0, util_1.isPositiveInteger)(v)) {
                throw new Error(`Expected positive integer but ${v} has type ${typeof v}`);
            }
            return signature_1.SignatureG1.encodePositiveNumberForSigning(v);
        };
    }
    /**
     ...
     * @param minimum
     */
    static integerToPositiveInt(minimum) {
     ...
     */
    static integerEncoder(minimum) {
        const f = Encoder.integerToPositiveInt(minimum);
        return (v) => {
            return signature_1.SignatureG1.encodePositiveNumberForSigning(f(v));
        };
    }
    /**
     ...
     */
    static positiveDecimalNumberToPositiveInt(maxDecimalPlaces) {
        if (!(0, util_1.isPositiveInteger)(maxDecimalPlaces)) {
     ...
     */
    static positiveDecimalNumberEncoder(maxDecimalPlaces) {
     ...
     * @param compress
     */
    static reversibleEncoderString(compress = false) {
        return (v) => {
            return signature_1.SignatureG1.reversibleEncodeStringForSigning(v, compress);
        };
    }
    /**
     ...
     * @param maxDecimalPlaces
     */
    static decimalNumberToPositiveInt(minimum, maxDecimalPlaces) {
        if (!(0, util_1.isPositiveInteger)(maxDecimalPlaces)) {
            throw new Error(`Maximum decimal places should be a positive integer but was ${maxDecimalPlaces}`);
        }
        const offset = Math.abs(minimum);
     ...
     * @param maxDecimalPlaces - The maximum decimal places
     */
    static decimalNumberEncoder(minimum, maxDecimalPlaces) {
     ...
     */
    static defaultEncodeFunc() {
        const te = new TextEncoder();
        return (v) => {
            // @ts-ignore
            return signature_1.SignatureG1.encodeMessageForSigning(te.encode(v.toString()));
        };
    }
    static ensureNumber(v) {
        if (typeof v !== 'number') {
            throw new Error(`Expected number but ${v} has type ${typeof v}`);
        }
    }
    static ensureCorrectDecimalNumberPlaces(n, maxDecimalPlaces) {
        const parts = n.toString().split('.');
        if (parts.length > 1 && parts[1].length > maxDecimalPlaces) {
            throw new Error(`Encoder was created with maximum decimal places ${maxDecimalPlaces} but was asked to encode ${n}`);
        }
    }
}
